...
chart_label = tk.Label(root, text="最近7天统计：", font=("Arial", 10))
chart_label.pack()

canvas = tk.Canvas(root, width=320, height=120, bg="white")
canvas.pack(pady=5)
...
def refresh_chart():
    ...
    for i in range(7):
        ...
        y1 = base_y - height
        y2 = base_y
        canvas.create_rectangle(x1, y1, x2, y2, fill="tomato")
        canvas.create_text((x1 + x2) // 2, base_y + 12, text=days[i][3:], font=("Arial", 8))
        canvas.create_text((x1 + x2) // 2, y1 - 5, text=str(counts[i]), font=("Arial", 8))
